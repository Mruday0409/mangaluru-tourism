...
export default function PlaceDetail() {
  ...
  return (
    <>
      <section className="detail-hero"><ScrollReveal><img className="detail-hero-image" src={place.image} alt={place.name} /></ScrollReveal><ScrollReveal delay={100}><div className="detail-hero-copy"><Link href="/places" className="eyebrow" style={{ color: "var(--sun)", textDecoration: "none" }}><ArrowLeft size={14} /> Back to places</Link><h1>{place.name}</h1><p>{place.description} Plan for the light, take the turn, and let the place be more than a photo stop.</p><div className="detail-meta"><span>{place.category}</span><span>{place.distance}</span><span>Field note / 0{place.slug.length % 7 + 1}</span></div><div className="page-route-spine detail-route-spine"><span>Route / {place.category}</span><div className="route-line" /><span>Open / 01</span></div></div></ScrollReveal></section>
